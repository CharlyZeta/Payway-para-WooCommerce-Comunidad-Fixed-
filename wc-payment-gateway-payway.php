...
class WC_Payment_Gateway_Payway extends WC_Payment_Gateway {
	...
	public function needs_setup() {
		...
		$sandbox_ready = !empty($options[WC_Payway_Config_Interface::FIELD_SANDBOX_SITE_ID]) 
						&& !empty($options[WC_Payway_Config_Interface::FIELD_SANDBOX_PUBLIC_KEY])
						&& !empty($options[WC_Payway_Config_Interface::FIELD_SANDBOX_PRIVATE_KEY]);

		$production_ready = !empty($options[WC_Payway_Config_Interface::FIELD_PRODUCTION_SITE_ID]) 
						&& !empty($options[WC_Payway_Config_Interface::FIELD_PRODUCTION_PUBLIC_KEY])
						&& !empty($options[WC_Payway_Config_Interface::FIELD_PRODUCTION_PRIVATE_KEY]);
	...
	public function is_available() {
		...
		$config_ready = $this->is_config_ready_for_checkout();
		$valid_for_use = $this->is_valid_for_use();
		$needs_setup = $this->needs_setup();

		// Debug for Sandbox mode
		if ( wc_payway_is_sandbox_enabled() ) {
			$logger = wc_payway_get_logger();
			$logger->debug( sprintf(
				'Payway Availability Check: parent_available: %s, config_ready: %s, valid_for_use: %s, needs_setup: %s, currency: %s',
				$is_available ? 'yes' : 'no',
				$config_ready ? 'yes' : 'no',
				$valid_for_use ? 'yes' : 'no',
				$needs_setup ? 'yes' : 'no',
				get_woocommerce_currency()
			));
		}

		return (bool) $is_available && $config_ready && $valid_for_use && !$needs_setup;
	}
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			if ( $request->get_success() ) {
				...
				// 3D Secure Interception
				if ( isset($request->is_challenge_required) && $request->is_challenge_required ) {
					$order->update_status('pending', __('Autenticación 3D Secure pendiente.', 'wc-gateway-payway'));
					WC_Payway_Meta::set_order_transaction_id( $order_id, $request->get_transaction_id() );

					$callback_url = site_url('/wc-api/payway_3ds_return/?order_id=' . $order_id);
					$redirect_url = $request->challenge_url . '?return_url=' . urlencode($callback_url);
