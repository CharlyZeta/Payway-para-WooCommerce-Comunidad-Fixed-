...
class WC_Payway_Meta implements WC_Payway_Meta_Interface {
	...
	/**
	 * Sets whether the order was authenticated via 3DS
	 *
	 * @param int $order_id
	 * @param string $value 'yes'|'no'
	 */
	public static function set_order_3ds_authenticated( $order_id, $value ) {
		update_post_meta(
			$order_id,
	...
	/**
	 * Gets 3DS authentication status
	 *
	 * @param int $order_id
	 * @return string
	 */
	public static function get_order_3ds_authenticated( $order_id ) {
		return get_post_meta(
			$order_id,
			self::get_full_meta_name( static::THREE_DS_AUTHENTICATED ),
			true
