...
class WC_Payway_Promotion_Factory {
	...
	public static function get_applicable_promotions() {
		...
		$current_time = current_time('mysql'); // Local WP Time
		...
		$query = $wpdb->prepare(
			...
			WHERE
				promos.is_active = 1
				AND promos.from_date <= %s
				AND promos.to_date >= %s
				AND (
					promos.applicable_days LIKE %s
					OR promos.applicable_days LIKE %s
					OR promos.applicable_days LIKE %s
					OR promos.applicable_days = %s
				)
			...
			$current_time,
			$current_time,
			'%' . $current_day . ',%',   // En medio o inicio con coma
			'%,' . $current_day . '%',   // Al final o con coma previa
			'% ' . $current_day . '%',   // Con espacio (por si acaso)
			(string) $current_day        // Solo el número
		);
